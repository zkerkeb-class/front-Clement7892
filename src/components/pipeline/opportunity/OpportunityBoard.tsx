...
const OpportunityBoard: React.FC<OpportunityBoardProps> = ({
  ...
  const getOpportunityEditUrl = (opportunityId: string) => {
    ...
    // Utiliser le clientId de l'opportunité si clientId n'est pas fourni
  ...
  const getAddOpportunityUrl = () => {
    if (!clientId) {
      // L'ajout depuis la vue globale redirige vers la liste des clients pour en sélectionner un
      return `/dashboard/pipeline/clients`;
    }
    // L'ajout depuis la vue par client a l'ID du client déjà disponible
